...
const part2 = (input) => {
  return R.sum(input.split('\n').map((line) => {
    ...
    const firstDigits = digits.map((digit,idx) => R.apply(Math.min,[line.indexOf(digit),line.indexOf(`${idx}`)].filter(idx => idx !== -1)));
    const lastDigits = digits.map((digit,idx) => R.apply(Math.max,[line.lastIndexOf(digit),line.lastIndexOf(`${idx}`)].filter(idx => idx !== -1)));
