...
const solve = (input) => {
  ...
  const partNumbers = inputLines.map((line,lidx) => {
    const numbers = ([...line.matchAll(/\d+/g)]).map((match) => {
      const hasAdjacentSymbol = [lidx-1,lidx,lidx+1].map(idx => {
        if(symbols[idx]) {
...
const part2 = (input) => {
  const inputLines = input.split('\n').filter(Boolean);
  const numbers = inputLines.map(line => ([...line.matchAll(/\d+/g)]).map((match) => ({number:parseInt(match[0]),start:match.index,end:match.index+match[0].length-1})));
  const gears = inputLines.flatMap((line,lidx) => ([...line.matchAll(/[*]/g)]).map((match) => {
