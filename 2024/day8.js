...
const solve = (input) => {
  ...
  const antinodes = ({x:x1,y:y1},{x:x2,y:y2}) => {
    ...
    return [{x:x2+dx,y:y2+dy},{x:x1-dx,y:y1-dy}].filter(inGrid);;
  }
  ...
  return R.uniq(R.flatten(R.map((antennas) => {
...
const part2 = (input) => {
  ...
  const antinodes = ({x:x1,y:y1},{x:x2,y:y2},mul=0) => {
    const {dx,dy} = {dx:x2-x1,dy:y2-y1};
    const nodes = [{x:x2+dx*mul,y:y2+dy*mul},{x:x1-dx*mul,y:y1-dy*mul}].filter(inGrid);
