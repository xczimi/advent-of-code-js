...
const parseInput = (input) => input.split('\n').filter(Boolean).map((line) => line.split(' ').map(x=>parseInt(x,10)));
const isSafe = (levels) => {
  const increase = levels[1]>levels[0] ? [1,2,3] : [-1,-2,-3];
  return !levels.slice(1).find((level,idx)=> !increase.includes(level - levels[idx]));
}
const solve = (input) => {
...
const part2 = (input) => {
  ...
  return reports.map(levels => {
    ...
    const increase = levels[1]>levels[0] ? [1,2,3] : [-1,-2,-3];
    const problems = levels.slice(1).map((level,idx)=> !increase.includes(level - levels[idx]));
